...
//    VIAJAR AL PROYECTO: se fueron a `backend/_manual.gs` (2026-09-22) con las otras 49
//    sondas. Apps Script analiza el proyecto ENTERO en cada ejecución y esas 2.497 líneas
//    —el 17,1 % de este fichero— no las invoca ninguna familia. Cómo se vuelven a subir
//    para medir: el ciclo de tres órdenes de la cabecera de `_manual.gs`.
...
function saveResponses_(p) {
  ...
  // ── DL-E49 §8 (2026-08-24) — AQUÍ VIVÍA EL RECHAZO PREVIO, Y SE RETIRA ───────────────
  //
  // Hasta hoy este manejador preguntaba `enr.estadoDeLasPartes` y, si constaba que este tutor
  // YA había enviado su parte, rechazaba con `PARTE_YA_ENVIADA` **antes de encolar nada** —
  // porque la regla del KMS (DL-E49 §6) descartaba esas respuestas en silencio.
  //
  // **Diego cambió el criterio el 2026-08-24**: *«Si edita algo se habilita de nuevo la
  // posibilidad de envío y se invalida el envío previo. Consta como no enviado hasta que le de a
  // enviar de nuevo.»* ⇒ el que ya envió **SÍ** sigue rellenando. El bloqueo del KMS se retiró
  // en el mismo cambio, y lo que ocurre ahora es que **su envío se invalida**
  // (`enr_invalidarParteDelQueEdita_`, en el despachador único del KMS).
  //
  // Se retira el rechazo **y su respaldo** (el que miraba `skipped_already_submitted` en la
  // respuesta): ese campo ya no lo emite nadie. Dejarlo sería vigilar algo que no puede ocurrir.
  //
  // `wizardTutorQueOpera_` **se queda**: sigue siendo quien identifica al tutor que contesta, y
  // ahora además es lo que permite al KMS saber **a quién** invalidarle la parte.
...
// ─── ⛔ LAS SONDAS `manual_*` YA NO VIVEN AQUÍ ────────────────────────────────────────
//
// Las 51 funciones `manual_*` de este fichero (2.497 líneas, el 17,1 %) se movieron a
// `backend/_manual.gs` el 2026-09-22 y ese fichero está en `.claspignore`: **NO viaja al
// proyecto**. Apps Script ANALIZA EL PROYECTO ENTERO en cada ejecución, así que ese 17 %
// se parseaba en las ~120 peticiones por hora del pulso y en todas las del camino vivo.
//
// ⛔ **ANTES DE ESCRIBIR UNA `manual_*` NUEVA, pregúntate quién la va a llamar.** Si es una
// SONDA DE DIAGNÓSTICO, va a `_manual.gs`. Si es TUBERÍA que algo del producto o del
// despliegue invoca por su NOMBRE —un disparador, un guion de publicación—, **tiene que
// viajar** y va en un fichero de módulo, aunque lleve el prefijo.
//
// Cómo se sube una sonda para medir y cómo se vuelve a bajar: el ciclo de TRES órdenes de
// la cabecera de `backend/_manual.gs`. ⛔ La tercera orden NO es opcional.
// ──────────────────────────────────────────────────────────────────────────────────────
