...
> **★ IDENTITY-BINDING — el binding token→tutor vive SERVER-SIDE (2026-06-11, findings #45). Pieza CANÓNICA del modelo de autorización del wizard.** Modelo de Diego (LA regla, cita literal): *"Cualquier tutor, en cualquier momento, recupera la solicitud desde admissions.kaleide.org metiendo SU email personal. Esto lleva implícito que se sabe SIEMPRE quién es el tutor que accede."* → la identidad del tutor NO puede vivir en el cliente.
>
> **Antes**: la identidad viajaba como `recovered_email` en el CLIENTE (sessionStorage/param). Tras la limpieza KAL-7 de la URL, un F5/incógnito/pestaña nueva la perdía → el wizard "olvidaba" quién era el tutor; `requireSignerContext_` (path a) ni resolvía al firmante sin ese campo. KAL-4 a medias (grupo del token, guardian del cliente).
>
> **Ahora**: al emitir el magic link, el wizard persiste el binding `enrEnrollmentGroups.recovery_guardian_email = <email del guardian al que se emitió>` server-side (`persistRecoveryBinding_`, en `sendMagicLink_`). Toda resolución posterior (`resumeSession_` / `getAdmissionState_` / `hydrateSession_` / `requireSignerContext_`) deriva la identidad del token via `effectiveRecoveredEmail_(clienteEmail, groupRow)` — precedencia `cliente → binding`; el `recovered_email` del cliente queda como compat secundario y **deja de ser necesario**. La identidad sobrevive a F5/incógnito/pestañas.
>
> **Reglas canónicas inmiscibles**:
> - El binding SOLO se escribe en EMISIÓN (server-side). NUNCA viene del payload del cliente (KAL-4 elevado a su forma correcta).
> - El binding se escribe en un Edit SEPARADO y TOLERANTE A FALLO, NUNCA empaquetado con la renovación del `resume_token` (si la columna `recovery_guardian_email` aún no existe → P72 silent reject AISLADO, la rotación del token no se afecta, degrada a group-scoped).
> - La rotación del token re-escribe el binding con él (mismo row): un token viejo no resuelve la fila → no llega al binding → caduca con el token.
> - Se almacena el EMAIL (no el `person_id`) porque ambos resolvers (wizard `resolveGuardianForRecovery_` + KMS `enr_resolveGuardianFromEmail_`) matchean por email → contrato KMS INTACTO (el wizard pasa el `recovered_email` binding-derived al `enr.wizardHydrate`; CERO cambio KMS).
> - **`magic_link_token` NO se reutilizó** para esto: está documentado como token alternativo (UNIQUE-parcial-WHERE-NOT-NULL) — meter un email chocaría con ese constraint si un guardian recupera varios grupos. Columna DEDICADA `recovery_guardian_email`. `magic_link_token` sigue vestigial.
>
> Test: `manual_testIdentityBinding` (a: emisión → binding == email + binding→guardian; b: resolución con SOLO el token → guardian + signing_context reconstituidos; c: token rotado → binding viejo inservible; d: email sin guardian → group-scoped intacto). AppSheet TODO: alta de `recovery_guardian_email` (`kis-app/docs/kms/appsheet-todo-2026-06-11.md` AT-IDBIND-01). Cross-ref: `kis-app/docs/kms/reports/2026-06-11-identity-binding.md` + findings #45 + data-navigation-chart fila 20.
