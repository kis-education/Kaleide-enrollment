...
El wizard tiene **11 steps canónicos** (no inventar otros — ver anti-patrones abajo). La firma (Steps 8-11) se autentica con un **`signing_token` por-firmante distinto** del `resume_token` de `/apply` — esa separación de **autorización** (dos bearer tokens) se preserva intacta; ver §"Dos bearer tokens canónicos del wizard" arriba + `Code.js:272`.

> **ENMIENDA UX — DL-E38 (2026-06-06, `kis-app/docs/kms/decisions/enr.md`): el wizard es UN flujo único continuo 1→11 de cara al usuario.** Lo que cambia respecto a la redacción previa de esta sección es **la capa UX/routing**, NO el modelo de seguridad. CLI 45 partió el wizard en dos rutas (`/apply` + `/sign`) que el usuario percibía como **inconexas**; DL-E38 corrige esa percepción sin tocar la autorización. Tres principios:
...
>
> **Lo que DL-E38 PRESERVA de CLI 45 (sin cambios):** la **firma es por-firmante y legalmente vinculante**; los **dos tokens siguen bajo el capó** (`resume_token` sesión-de-grupo + `signing_token` por-firmante); el `enrollment_group_id` y el signer se derivan SIEMPRE **server-side del token, NUNCA del payload** (KAL-4 IDOR). El resume resuelve el contexto de firma del guardian server-side a partir del `resume_token`, no de un email ni de un campo del cliente. Cambia la **UX/routing**, NO la identidad por-firmante.
>
> Items de build: **P215** (`resumeSession_` devuelve estado real + disponibilidad de firma + contexto del guardian) · **P216** (frontend: Step 7 estado real + avance state-driven) · **P217** (bridge Step 7 → firma). Prerequisito **P211** (sin el fix del `signing_token` PackedUUID dashless la firma NI resuelve). Cross-ref DL-E37 («Acciones disponibles» locus de estado) + P200/P201 (emisión del `signing_token`).
