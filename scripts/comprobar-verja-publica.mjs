#!/usr/bin/env node
/**
 * ②2 — las tres entradas públicas del asistente pasan por UNA sola verja, y falla cerrado.
 *
...
try {
  ...
  if (fallos.length) motivo = `${fallos.length} infracción(es): ${fallos.join(' · ')}`
    ...
    console.log('  ✓ crear solicitud · reconocer familia · recuperar enlace: las tres pasan por ella')
